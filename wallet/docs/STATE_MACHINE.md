...
source==='nfc':
  ...
  selectMint
       -> rank eligible candidates: request mint membership, enough balance,
          higher balance, then existing order (stable ties)
  enterAmount   -> reject amountless requests (NFC_READ_FAILED)
  navigateToPaymentRequest + nfc
       -> runNfcWriteBackEffect -> executeNfcSend -> token written to tag
       -> sendComplete
```

Before the first transition, NFC reads the request's single unit. When it differs
...
retains the ordinary unit mismatch hard-stop. A rejected switch releases the
session with `NFC_READ_FAILED`. Non-NFC flows keep their existing hard-stop.
The selecting phase covers currency selection, and token creation receives the
resolved unit explicitly. No conversion, trust change, or mint-info fetch is
performed. Candidates expose no fee data, so ranking does not consider fees.

Device evidence still required: a real numo tap in **sat** and **usd**, including
a sat-active wallet paying a funded USD request, an unfunded accepted-mint
failure, and the Android selecting copy/Close control. Unit tests do not prove
native NFC timing, write-back, or settlement. No real-funds or simulator E2E was
run for this change.
