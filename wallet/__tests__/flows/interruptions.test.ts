...
describe("interruptions — reset", () => {
  ...
  it("reset clears scan provenance left by a deduplicated input", async () => {
    ...
    // The distinct-scan contract normally consumes this immediately. Use the
    // same pending payload so dedup returns before EXECUTE can consume source.
    await tm.machine.scan?.(input, { source: "gallery" });
    tm.machine.reset();

    await tm.machine.execute(INPUTS.mintUrl);
    tm.assertStep("openMint");
    expect(tm.machine.getContext().source).toBeUndefined();
    const resolved = tm.notificationCalls.filter(
      (call) => call.key === "onScanResolved",
    );
    expect(resolved.at(-1)?.data).toMatchObject({ source: undefined });
  });
});
...
describe("interruptions — table-driven scenarios", () => {
  it.each([RESET_MID_FLOW, RE_EXECUTE_OVERRIDE])("$name", async (scenario) => {
