...
export type MeasuredReplyMatch = {
  eventId: string;
  originalIndex: number;
  sortedIndex: number;
  skeletonIndex: number | null;
...
export type MeasuredSortResult = {
  replies: FeedEvent[];
  matches: MeasuredReplyMatch[];
};

export function sortRepliesByMeasuredHeights(
  replies: readonly FeedEvent[],
  skeletonHeights: readonly number[],
  replyHeights: ReadonlyMap<string, number>
): MeasuredSortResult {
  const matchCount = Math.min(MAX_REPLY_SKELETON_COUNT, skeletonHeights.length, replies.length);

  const pool = replies.map((event, originalIndex) => ({
    event,
    originalIndex,
    height: replyHeights.get(event.id),
  }));
  ...
  for (let slot = 0; slot < matchCount; slot += 1) {
    const targetHeight = skeletonHeights[slot];
    if (targetHeight == null) break;
    ...
    for (let i = 0; i < pool.length; i += 1) {
      ...
      if (candidate.height == null) continue;
      const heightDelta = Math.abs(candidate.height - targetHeight);
      const score = heightDelta * 1000 + candidate.originalIndex;
      if (score < bestScore) {
        bestPoolIndex = i;
        bestScore = score;
      }
    ...
    const [best] = pool.splice(bestPoolIndex, 1);
    selected.push({
      event: best.event,
      originalIndex: best.originalIndex,
      measuredHeight: best.height as number,
      skeletonIndex: slot,
      skeletonHeight: targetHeight,
    });
  }

  pool.sort((a, b) => a.originalIndex - b.originalIndex);

  const orderedEvents: FeedEvent[] = [
    ...selected.map((entry) => entry.event),
    ...pool.map((entry) => entry.event),
  ];

  const matches: MeasuredReplyMatch[] = [
    ...selected.map((entry, sortedIndex) => ({
      eventId: entry.event.id,
      originalIndex: entry.originalIndex,
      sortedIndex,
      skeletonIndex: entry.skeletonIndex,
      skeletonHeight: entry.skeletonHeight,
      measuredHeight: entry.measuredHeight,
      heightDelta: entry.measuredHeight - entry.skeletonHeight,
    })),
    ...pool.map((entry, tailIndex) => {
      const measuredHeight = entry.height ?? 0;
      return {
        eventId: entry.event.id,
        originalIndex: entry.originalIndex,
        sortedIndex: selected.length + tailIndex,
        skeletonIndex: null,
        skeletonHeight: null,
        measuredHeight,
        heightDelta: null,
  ...
  return { replies: orderedEvents, matches };
}
