...
function WaitingTag() {
  ...
  const tagStyle = [styles.bearerTag, { backgroundColor: opacity(foreground, 0.08) }];
  const tagTextStyle = {
    color: opacity(foreground, 0.55),
  };
...
function NearPayPeerRow({ peer, onSelect }: NearPayPeerRowProps) {
  const handlePress = () => onSelect(peer);
...
export function NearPayPeerListScreen() {
  ...
  const peers = filterFreshBLEPeers(blePeers, peerFreshnessNow);
  ...
  const summaryStyle = [styles.summary, { borderBottomColor: opacity(foreground, 0.08) }];
  const summaryTextStyle = {
    color: opacity(foreground, 0.6),
  };
  const emptyIconColor = opacity(foreground, 0.3);
  const emptyTitleStyle = {
    color: opacity(foreground, 0.5),
    textAlign: 'center' as const,
  };
  const emptyTextStyle = {
    color: opacity(foreground, 0.35),
    textAlign: 'center' as const,
  };
