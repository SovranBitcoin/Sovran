...
export function useAiSend() {
  ...
  const navigateToTopUp = (pendingMessage: string) => {
    if (!nostrKeys?.pubkey) {
   ...
   */
  const streamIntoPlaceholder = async (params: {
  ...
  const sendInner = async (userMessage: string) => {
   ...
   */
  const retryInner = async (messageId: string) => {
    if (!apiKey) {
      staticPopup('no-api-key');
      return;
    }
