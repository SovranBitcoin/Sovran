...
const PATCHES = [
  ...
  //
  // Mirror of the iOS NUT_PAYLOAD_ROUTE patch in patch-bitchat-imports.js.
  // The intercept sits BEFORE NoisePayload.decode because — unlike iOS, where
  // unknown types fall to a fail-silent guard — Android's decode rejects
  // unknown type bytes outright. The FULL decrypted payload (type byte
  // included) goes to NutPayloadRelay; all Cashu semantics live in JS.
  {
    file: 'mesh/MessageHandler.kt',
    name: 'NUT_PAYLOAD_ROUTE',
    anchor:
    ...
    replacement:
      ...
      '            // full typed payload to the bridge relay before NoisePayload.decode\n' +
      '            // (which rejects unknown type bytes) — the native layer is a dumb\n' +
      '            // byte pipe; Cashu semantics live in JS.\n' +
      '            if (com.bitchat.android.ecash.NutPayloadRelay.containsType(decryptedData[0].toInt() and 0xFF)) {\n' +
      '                com.bitchat.android.ecash.NutPayloadRelay.receive(\n' +
      '                    peerID,\n' +
      '                    decryptedData,\n' +
  ...
  // Raw-bytes sibling of sendNoisePayloadToPeer for the Nut Drop vendor
  // payloads: the typed payload arrives fully encoded from the bridge (type
  // byte included), so it is encrypted as-is instead of via
  // NoisePayload.encode(). Same packet shape, signing, and broadcast path as
  // upstream's own Noise payload sends. Requires an established Noise session
  // (encrypt throws otherwise, caught + logged like upstream) — the JS layer
  // establishes one via startBLEPrivateChat before soliciting.
  {
    file: 'mesh/BluetoothMeshService.kt',
    name: 'NUT_RAW_NOISE_SEND',
    anchor:
      /    private fun sendNoisePayloadToPeer\(payload: NoisePayload, recipientPeerID: String, label: String\) \{/,
    replacement:
      '    // [sovran] raw-bytes sibling of sendNoisePayloadToPeer — sends the Nut\n' +
      '    // Drop vendor Noise payloads (0xA0–0xA3) through the same packet path.\n' +
      '    internal fun sendRawNoisePayload(typedPayload: ByteArray, recipientPeerID: String, label: String) {\n' +
      '        serviceScope.launch {\n' +
      '            try {\n' +
      '                val encrypted = encryptionService.encrypt(typedPayload, recipientPeerID)\n' +
      '                val packet = BitchatPacket(\n' +
      '                    version = 1u,\n' +
      ...
      '                Log.d(TAG, "📤 Sent $label to $recipientPeerID (${typedPayload.size} bytes)")\n' +
      '            } catch (e: Exception) {\n' +
