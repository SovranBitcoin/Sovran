...
| F06 | **Contract done 2026-09-13**; screens adopt per surface | `read: ReadStatusMeta` on feed/thread/notification results, set by [facadeFeedClient](app/features/feed/data/facadeFeedClient.ts) | [facadeFeedClient test](app/__tests__/facadeFeedClient.test.ts): exhaustion → `unavailable`, no tiers → `disabled`, untouched → ok; loading still terminates |
