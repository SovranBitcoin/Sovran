 ...
 *
 * When a BIP-321 URI has only unsupported parameters (like an on-chain
 * address with no lightning/cashu fallback), the parser returns
 * type: 'bip321' with empty options — the machine routes this to error.
 */
describe('parsePaymentInput — BIP-321', () => {
  ...
  it('returns bip321 type when no supported options found', () => {
    ...
    // We don't support on-chain payments, so options are empty.
    // type is 'bip321' (not 'unknown') because we recognized the container format.
