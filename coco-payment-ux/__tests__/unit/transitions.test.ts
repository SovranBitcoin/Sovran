...
describe('transition — EXECUTE', () => {
  ...
  it('works from any step (not just idle)', () => {
    ...
    const result = tx('enterAmount', { ...idle, amount: 50 }, {
      type: 'EXECUTE',
...
describe('transition — REQUEST_MINT_SELECTOR', () => {
  it('routes to selectMint with candidates', () => {
    const result = tx('enterAmount', { ...idle, destination: 'sendEcash' }, {
...
describe('transition — AMOUNT_ENTERED', () => {
  it('advances flow after amount is entered with mint', () => {
    ...
    const ctx: FlowContext = {
      ...
      intent: { type: 'meltLightningAddress', option: { kind: 'lightningAddress', value: 'user@example.com', source: 'standalone' } },
...
describe('transition — PROOFS_CHOSEN', () => {
  it('routes to confirmSend for ecash destination', () => {
    const ctx: FlowContext = {
      ...
      mintUrl: MINT1,
      amount: 100,
      intent: { type: 'receiveToken', option: { kind: 'ecashToken', value: 'tok', source: 'standalone' } },
    };
