...
Every user-facing string returned from `coco-payment-ux` is localized. Reason codes, error messages, and execution state messages are all translated based on the locale configured on the [provider](/guide/getting-started#provider).
