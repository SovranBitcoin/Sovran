...
test("copy strings, dependencies, new files and deletions count as app changes", (t) => {
  for (const change of [
    ...
    ({ root }) => unlinkSync(join(root, "app/features/receive/ReceiveScreen.tsx")),
  ]) {
...
test("stamps record the commit and changed files are reported against it", (t) => {
  ...
  assert.deepEqual(appFilesChangedSince(root, stamp.gitSha), ["app/features/receive/ReceiveScreen.tsx"]);
  assert.equal(appSourceFingerprint(join(tmpdir(), "definitely-not-a-repo-" + process.pid)), undefined);
});
...
test("classification distinguishes current, outdated, unverified, withdrawn, missing and unknown", () => {
  ...
  assert.equal(classifyCapture({ ...capture, appSource: { fingerprint: "f1" } }, "f1"), "current");
  assert.equal(classifyCapture({ ...capture, appSource: { fingerprint: "f1" } }, "f2"), "outdated");
  assert.equal(classifyCapture(capture, "f1"), "unverified");
  assert.equal(classifyCapture({ ...capture, freshness: "stale" }, "f1"), "withdrawn");
  assert.equal(classifyCapture({ ...capture, availability: "unavailable" }, "f1"), "withdrawn");
  assert.equal(classifyCapture({ run: null, sha256: null }, "f1"), "missing");
  assert.equal(classifyCapture(undefined, "f1"), "withdrawn");
  assert.equal(classifyCapture({ ...capture, appSource: { fingerprint: "f1" } }, undefined), "unknown");
});
