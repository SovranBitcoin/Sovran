import { FRAME_IDS, SCENE_PRESETS } from '../../scripts/lib/phone-frame.mjs';
...
export function validateRecipe(value) {
  if (!keys(value, ['version', 'id', 'title', 'headline', 'subtitle', 'background', 'width', 'height', 'preset', 'phones', 'brand', 'draft']) || value.version !== 1)
  ...
  return { version: 1, id: value.id, title: value.title, headline: value.headline, subtitle: value.subtitle,
    background: value.background, width: value.width, height: value.height, preset: value.preset, phones,
    ...(brand ? { brand } : {}), ...(value.draft ? { draft: true } : {}) };
}
