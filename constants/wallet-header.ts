/**
 * Shared wallet header layout constants and dimension helpers.
 * Used by the wallet tab layout and any screens that need consistent header sizing.
 *
 * In components, prefer useWindowDimensions() + getHeaderTitleWidthFromWidth(width)
 * over getHeaderTitleWidth() so layout reacts to orientation/resize (see building-native-ui).
 */
...
/** Use when hook context not available (e.g. outside component). Prefer getHeaderTitleWidthFromWidth(useWindowDimensions().width) in components. */
