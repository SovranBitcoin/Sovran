...
- **Coco polling — patch removed after safety review:** restored unpatched `@cashu/coco-core@2.0.0`. The patch applied every polling failure to a mint-wide cooldown without exposing that delay to the UI. A deterministic public-API reproduction showed repeated HTTP 400 responses on an old quote delaying a new, healthy proof-state request by ten minutes after backoff reached its cap. A longer `Retry-After` could delay unrelated checks beyond five minutes, including after the original subscription was removed. The entire Coco patch, its Bun registration, and its patch-specific contract tests were removed at the user's request, including the bundled teardown changes. Future retry work needs error classification, isolation between operations, visible degraded status, and responsiveness tests before adoption. Other thermal changes remain in place.
...
Dependency versions are unchanged. The lockfile no longer registers a Coco patch; other dependency patches remain unchanged.

### Verification and remaining device checks

After patch removal: all 82 wallet suites / 1,209 tests passed, along with wallet and app iOS/Android TypeScript checks. The installed Coco entry file matches the published unpatched `2.0.0` package byte-for-byte. The lockfile diff only removes the Coco patch registration.

Before removal, the patch-specific regression coverage exercised actual installed Coco through its public subscriptions API with in-memory repositories and mocked HTTP, including a later paid observation after 429, increasing backoff, HTTP-date cooldown shared with a new subscription, pause/resume, closing a mint, late request completion, proof-state retry, and final unsubscription. Those tests did not establish acceptable responsiveness for unrelated operations and were removed with the patch. App tests cover connectivity cadence/hysteresis, lifecycle cleanup, motion deadband, disabled logger work, and completed indicator deactivation. Design-system snapshots are unchanged.

Before patch removal, completed checks: 355 app suites / 3,595 tests / 160 snapshots passed; 83 wallet suites / 1,212 tests passed, followed by the expanded seven-case polling contract suite. App iOS/Android and wallet TypeScript checks passed, as did changed-file formatting, knip, and `git diff --check`. Focused ESLint had no errors (one existing `require()` warning; native patch scripts are excluded by the app lint configuration and were checked with Node). Structural analysis completed with its repository-wide findings.
