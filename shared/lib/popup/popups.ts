...
import { showActionSheet, showCustomToast } from './bridge';
...
// ---------------------------------------------------------------------------
// Payment status sheet (live pending/confirmed)
// ---------------------------------------------------------------------------
...
type PaymentStatusCase = {
  message: string;
  /** Pending submessage. Omit if variant only shows confirmed (e.g. send). */
  submessagePending?: string;
  /** Confirmed submessage. Receives amount/unit for formatting. */
...
/** Toggle between 'toast' (compact banner) and 'sheet' (full bottom sheet). */
const PAYMENT_STATUS_DISPLAY: 'toast' | 'sheet' = 'toast';

/**
 * Payment status popup (live pending/confirmed).
 * Receive: mint-quote:added (PAID) → mint-quote:redeemed.
 * Send: send:finalized only (when recipient redeems).
 * Melt: melt-op:finalized (v3 melt flow).
 */
