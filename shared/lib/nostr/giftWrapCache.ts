...
const cache = createPubkeyScopedCache<UnwrappedDM>({
  ...
  log: nostrLog,
  validate: (v): v is UnwrappedDM =>
    !!v && typeof (v as UnwrappedDM).senderPubkey === 'string',
});

export const hydrateGiftWrapCache = cache.hydrate;
export const getCachedUnwrap = cache.get;
export const putUnwrap = cache.put;
export const isKnownFailedUnwrap = cache.isKnownFailed;
export const markUnwrapFailed = cache.markFailed;
export const clearGiftWrapCache = cache.clear;
export const evictGiftWrapCacheFromMemory = cache.evictFromMemory;
