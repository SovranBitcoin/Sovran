...
import { decode, defaultCredentialTypes, encode, } from "ts-mls";
...
import { KEY_PACKAGE_CIPHER_SUITE_TAG, KEY_PACKAGE_CLIENT_TAG, KEY_PACKAGE_EXTENSIONS_TAG, KEY_PACKAGE_KIND, KEY_PACKAGE_MLS_VERSION_TAG, KEY_PACKAGE_RELAYS_TAG, } from "./protocol.js";
/**
...
export function createDeleteKeyPackageEvent(options) {
    ...
    const ids = events.map((e) => {
        ...
        if (e.kind !== KEY_PACKAGE_KIND) {
            throw new Error(`Event ${e.id} is not a key package event (kind ${e.kind} instead of ${KEY_PACKAGE_KIND})`);
        }
        return e.id;
    });
    return {
