...
describe('buildChunkPlan (real authoring tree)', () => {
  ...
  test('matches the fresh-matrix invariant: 125 scenarios, 213 pairs', () => {
    expect(plan.isOk()).toBe(true);
    const value = plan._unsafeUnwrap();
    expect(value.scenarioCount).toBe(125);
    expect(value.expectedPairKeys.length).toBe(213);
    expect(new Set(value.expectedPairKeys).size).toBe(213);
  });
