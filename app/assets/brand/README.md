...
## App, website, and press ownership

`source/symbol.svg` remains the single editable S, not a newly drawn mark.
`source/wordmark.svg` owns the separate outlined lettering. Do not copy either
geometry into a component or create another master for the website.

| Consumer | Derivation |
| --- | --- |
| App/store icon, Android adaptive icon, widget, splash and Expo favicon | `scripts/brand-assets.mjs`, selected by `app/app.json`, `app/app.config.js`, `app/targets/widget/expo-target.config.js` and `app/app/_layout.tsx` |
| Press concept artwork and feature graphics | `scripts/lib/marketing-render.mjs` calls `composeBrand` for the shared wordmark lockup |
| Website header/footer, favicon and OG scene mark | `scripts/site-brand.mjs` writes `site/public/sovran-mark.svg` from the same master |
| Website download QR logo | `scripts/site-brand.mjs` replaces only the logo slot in `site/public/downloads/download-qr.svg` |

Run `node scripts/site-brand.mjs` from the repository root after editing the
symbol, and `node scripts/site-brand.mjs --check` to detect drift. This separate,
dependency-free command deliberately stays outside native install/EAS hooks:
`site/` is excluded from native archives. Native asset generation is unchanged.
The website retains its original 621-square viewport and white foreground; the
QR retains its reviewed grid, mask, badge, placement and destination encoding.
The QR grid is still a manually imported artifact, not generated by this command.

Before replacement, the copied web mark and canonical master had identical alpha
pixels at 24px and 32px. At 621px and 2048px, alpha RMSE was respectively 0.0973
and 0.0479 on the 0-255 scale, attributable to existing path rounding. The master
...
The repository's [LICENSE](../../../LICENSE) is Mozilla Public License 2.0
(MPL-2.0). The website's
[fast-squircle MIT notice](../../../site/public/licenses/fast-squircle.txt)
applies to the adapted phone-frame construction, not to all project code or
the Sovran logo. Preserve that notice and Mona Sans's SIL OFL notice; moving
third-party code does not relicense it. See the
[website license clarification](../../../site/public/licenses/README.txt).
This asset guide does not grant trademark rights; MPL-2.0 section 2.3 excludes
such grants except as needed for its notice requirements.
