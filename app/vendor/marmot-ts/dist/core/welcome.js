...
import { mlsMessageDecoder, mlsMessageEncoder, } from "ts-mls/message.js";
...
export async function readWelcomeGroupInfo({ welcome, keyPackage, ciphersuiteImpl, }) {
    ...
    if (isRumorLike(welcome))
        welcome = getWelcome(welcome);
    const keyPackageRef = await makeKeyPackageRef(keyPackage.publicPackage, ciphersuiteImpl.hash);
    const initPrivateKey = await ciphersuiteImpl.hpke.importPrivateKey(keyPackage.privatePackage.initPrivateKey);
    let groupSecrets;
    try {
        groupSecrets = await decryptGroupSecrets(initPrivateKey, keyPackageRef, welcome, ciphersuiteImpl.hpke);
    }
    catch (err) {
    ...
    if (!groupSecrets)
        throw new Error("Failed to decrypt group secrets: key package does not match this welcome");
    ...
    const groupInfo = await decryptGroupInfo(welcome, groupSecrets.joinerSecret, pskSecret, ciphersuiteImpl);
    if (!groupInfo)
        throw new Error("Failed to decrypt group info from welcome");
    return groupInfo;
}
/**
