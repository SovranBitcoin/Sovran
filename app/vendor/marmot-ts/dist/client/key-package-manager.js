...
import { createDeleteKeyPackageEvent, createKeyPackageEvent, getKeyPackageReference, getKeyPackageRelays, } from "../core/key-package-event.js";
...
export class KeyPackageManager extends EventEmitter {
    ...
    constructor(options) {
        ...
        this.store.on("keyPackageUpdated", (kp) => this.emit("keyPackageUpdated", kp));
        this.store.on("keyPackageRemoved", (ref) => this.emit("keyPackageRemoved", ref));
    }
    // ---------------------------------------------------------------------------
    ...
    async purge(refs) {
        ...
        if (allEventIds.length > 0) {
            ...
            this.#log("published %s delete event for %d key packages", signed.id, allEventIds.length);
        }
