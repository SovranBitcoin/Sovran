...
export class MarmotClient extends EventEmitter {
    ...
    constructor(options) {
        ...
        this.historyFactory = ("historyFactory" in options ? options.historyFactory : undefined);
        // Set the media factory if its set in the options
        this.mediaFactory = ("mediaFactory" in options ? options.mediaFactory : undefined);
    }
    ...
    async importGroupFromClientState(state) {
    ...
    async createGroup(name, options) {
        ...
        await this.groupStateStore.set(clientState.groupContext.groupId, stateBytes);
