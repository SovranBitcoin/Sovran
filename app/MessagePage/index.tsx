...
export default function ModalScreen() {
  ...
  const { profiles, search, setSearch, messages, addMessage, currentProfile } =
    useNostr();
  ...
  const enrichedTransactions = Object.values(groupedTransactions).map(
    (group) => {
  ...
  const esimsWithRequest = esims.filter((esim) =>
    currentTransactions?.transactions?.some(
      (transaction) =>
        transaction.request && transaction.request === esim.request
    )
  );
  ...
  const vpnsWithRequest = vpn.filter((esim) =>
    currentTransactions?.transactions?.some((tx) => {
      return (
        tx.request &&
        esim.payment_request &&
        tx.request === esim.payment_request
      );
    })
  );
  ...
  const filteredMessages = messages
    ...
    .reduce((unique, msg) => {
      return unique.find((item) => item.id === msg.id)
        ? unique
        : [...unique, msg];
    }, []);
  ...
  const timelimeItems = [
    ...
    const getDate = (item) => {
      ...
      return (
        item?.date ||
        item?.order?.packageList?.[0]?.createTime ||
        vpnDate ||
