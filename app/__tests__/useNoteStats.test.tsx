...
it('is absent until a source counts the note, loading while a backfill is pending, cached once counts land', () => {
  ...
  act(() => {
    mockCache.ingestNoteStats(
      { [N1]: { likes: 3, reposts: 1, replies: 2, zaps: 0, satsZapped: 21 } },
      'relay'
    );
...
it('re-renders only for its own note id', () => {
  ...
  act(() => {
    mockCache.ingestNoteStats(
      { [N1]: { likes: 9, reposts: 0, replies: 0, zaps: 0, satsZapped: 0 } },
      'nagg'
    );
  });
  ...
  act(() => {
    mockCache.ingestNoteStats(
      { [N2]: { likes: 1, reposts: 0, replies: 0, zaps: 0, satsZapped: 0 } },
      'nagg'
    );
  });
...
it('a nagg count is never overwritten by a later relay lower bound', () => {
  ...
  act(() => {
    mockCache.ingestNoteStats(
      { [N1]: { likes: 40, reposts: 0, replies: 0, zaps: 0, satsZapped: 0 } },
      'nagg'
    );
  });
  act(() => {
    mockCache.ingestNoteStats(
      { [N1]: { likes: 2, reposts: 0, replies: 0, zaps: 0, satsZapped: 0 } },
      'relay'
    );
  });
