...
| `@routstr/sdk@0.4.6` | (1) `isTorContext` reads `window.location.hostname` behind a `typeof window` guard. React Native defines `window` but not `location`, so the guard passes and the read throws inside `findNextBestProvider` — its `catch` returns null, and the SDK gives up after ONE provider. A node answering 404 for a model it does not serve ended the whole request with eleven candidates untried. (2) `isNetworkErrorMessage` knows the Chrome, Firefox, Safari and Bun spellings of a failed fetch and none of React Native's, so `_makeRequest` rethrew every mobile network fault instead of routing it through `_handleErrorResponse` — no `X-Cashu` change reclaim, no original-token recovery, no failover, and the request's sats left stranded on the node. Guarded by `app/__tests__/routstrSdkPatch.test.ts`. | Each hunk: the SDK's browser build stops assuming `window` implies `window.location`; `isNetworkErrorMessage` recognises React Native |
