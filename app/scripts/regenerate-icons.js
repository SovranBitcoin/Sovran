...
/* global __dirname */
/**
 * Refreshes assets/icons/generated.json from the Iconify registry in
 * assets/icons/index.tsx and locally authored internal SVGs.
 * Run explicitly with: node scripts/regenerate-icons.js
 */
...
(async () => {
  ...
  const generatedNames = new Set(entries.map(([name]) => name));
  const missing = icons.filter((name) => !generatedNames.has(name));
  if (missing.length > 0) {
    throw new Error(`Registry unchanged: missing ${missing.join(', ')}`);
  }
  ...
  fs.writeFileSync(OUT_FILE, JSON.stringify(Object.fromEntries(entries), null, 2) + '\n', 'utf-8');
